...
hb.network <- function(hbonds) {
  ...
  for (i in 1:n.vertices) {
    ...
    hoh <- unique(c(df$donor[df$donor_amino == 'SOL'], df$acceptor[df$acceptor_amino == 'SOL']))
    hoh <- hoh[hoh != id]
...
find.swb <- function(hbonds) {
  ...
  nucles <- nucles[!sapply(nucles, function(nucle) {
    ...
    all(df$donor_amino != 'SOL') & all(df$acceptor_amino != 'SOL')
  })]
...
unique.waters <- function(hbonds, count = TRUE) {
  all.waters <- c(hbonds$donor[hbonds$donor_amino == 'SOL'], hbonds$acceptor[hbonds$acceptor_amino == 'SOL'])
